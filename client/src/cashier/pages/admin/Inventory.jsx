...
export default function Inventory() {
    ...
    const openEdit = (item) => {
        ...
        setFormData({ name: item.name, description: item.description || '', price: item.price, category: item.category, quantity: item.quantity });
        setImageFile(null);
    ...
    const handleEdit = async (e) => {
        ...
        if (imageFile) fd.append('image', imageFile);
        try {
            await api.put(`/menu/${selectedItem._id}`, fd, { headers: { 'Content-Type': 'multipart/form-data' } });
        ...
        <form onSubmit={onSubmit}>
            <div className="modal-body">
                ...
                <div className="form-group">
                    ...
                    <input type="number" value={formData.quantity} onChange={(e) => setFormData({ ...formData, quantity: e.target.value })} placeholder="Available stock" min="0" required />
                </div>
                <div className="form-group">
                    <label>Category</label>
                    <input type="text" value={formData.category} onChange={(e) => setFormData({ ...formData, category: e.target.value })} placeholder="e.g. Pizza, Burgers, Drinks" required />
                </div>
                <div className="form-group">
                    <label>Price (₹)</label>
                    <input type="number" value={formData.price} onChange={(e) => setFormData({ ...formData, price: e.target.value })} placeholder="Price in rupees" min="0" step="0.01" required />
                </div>
            </div>
            <div className="modal-footer">
                <button type="button" className="btn-reset" onClick={resetForm}>Reset</button>
                <button type="submit" className="btn-add-item">{submitLabel}</button>
            </div>
        </form>
    );
    ...
    return (
        <div>
                        ...
                        {items.map((item) => (
                            <div key={item._id} className="inventory-card">
                                {item.image ? (
                                    ...
                                    <div className="inventory-card-img" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '3rem', color: 'var(--text-muted)', background: 'var(--bg-input)' }}>🍴</div>
                                )}
                                <div className="inventory-card-body">
                                    ...
                                    <div className="inventory-card-stock">Stock: <strong>{item.quantity}</strong> units</div>
                                    <div className="inventory-card-actions">
                                        <button className="btn-edit" onClick={() => openEdit(item)}><FiEdit2 size={13} /> Edit</button>
                                        <button className="btn-delete" onClick={() => openDelete(item)}><FiTrash2 size={14} /></button>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            <button className="fab-add" onClick={() => { resetForm(); setShowAddModal(true); }}><FiPlus size={24} /></button>
