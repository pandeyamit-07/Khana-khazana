...
export default function LowStock() {
    ...
    const handleSave = async (e) => {
        ...
        try {
            ...
            await api.put(`/menu/${editItem._id}`, fd, {
        ...
        <div>
            ...
            <div className="low-stock-page">
                ...
                <div className="low-stock-header">
                    ...
                    <button className="btn-refresh-stock" onClick={fetchLowStock}>
                        Refresh
                    </button>
                </div>
                ...
                {loading ? (
                    ...
                    <div className="ls-empty">
                        <FiPackage size={48} className="ls-empty-icon" />
                        <h3>All good! 🎉</h3>
                    ...
                    <>
                        <div className="ls-count-bar">
                            <span>
                                <strong>{items.length}</strong> item{items.length !== 1 ? 's' : ''} need restocking
                            </span>
                        </div>
                            ...
                            {items.map((item) => {
                                ...
                                return (
                                    <div key={item._id} className={`ls-card ls-card--${level}`}>
                                        {/* Stock level badge */}
                                        <div className={`ls-level-badge ls-level--${level}`}>
                                        ...
                                        <div className="ls-card-img-wrap">
                                            {item.image ? (
                                                <img src={item.image} alt={item.name} className="ls-card-img" />
                                            ) : (
                                                <div className="ls-card-img-placeholder">🍴</div>
                                            )}
                                        </div>
                                        ...
                                        <div className="ls-card-body">
                                            ...
                                            <div className="ls-stock-row">
                                                ...
                                                <div className="ls-stock-bar-wrap">
                                                    <div
                                                        className={`ls-stock-bar ls-bar--${level}`}
                                                        style={{
                                                            width: `${Math.min(100, (item.quantity / LOW_STOCK_THRESHOLD) * 100)}%`,
                                                        }}
                                                    />
                                                </div>
                                                <span className={`ls-qty ls-qty--${level}`}>
                                                    {item.quantity}
                                                </span>
                                            </div>
                                        </div>
                        ...
                        <form onSubmit={handleSave}>
                            <div className="modal-body">
                                ...
                                <div className="form-group">
                                    <label>
                                        Quantity (Stock)
                                        <span className="ls-qty-hint"> — currently {editItem.quantity}</span>
                                    </label>
                                    <input
                                        type="number"
                                        ...
                                        style={{
                                            border: '2px solid',
                                            borderColor:
                                                Number(editForm.quantity) >= 10 ? '#22c55e'
                                                    : Number(editForm.quantity) <= 3 ? '#ef4444'
                                                        : '#f97316',
                                        }}
                                    />
                                    {Number(editForm.quantity) >= 10 && (
                                        <p className="ls-qty-ok">✅ Will clear the low stock alert</p>
                                    )}
                                </div>
                                <div className="form-group">
                            ...
                            <div className="modal-footer">
                                ...
                                <button type="submit" className="btn-add-item" disabled={saving}>
                                    <FiSave size={14} style={{ marginRight: 4 }} />
